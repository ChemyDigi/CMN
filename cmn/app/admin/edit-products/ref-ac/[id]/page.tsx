...
export default function EditACRefForm() {
  ...
  const fetchProduct = async () => {
    try {
      const docRef = doc(db, "AC-Ref", productId);
  ...
  const handleSubSelect = async (files: FileList | File[]) => {
    ...
    const totalExisting = existingSubImages.length + subImages.length;
  ...
  const removeExistingSubImageAt = (index: number) => {
  ...
  const onSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      // Combine existing and new sub images
      const allSubImages = [...existingSubImages, ...newSubImageUrls];

      await updateDoc(doc(db, "AC-Ref", productId), {
        serialId,
        category,
        productName,
        brand,
    ...
    <div className="max-w-6xl mx-auto p-4 text-gray-800 text-sm">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Edit AC & Refrigerator</h1>
        </div>
        <p className="text-gray-600 ml-4 text-xs">Update product information</p>
      </div>
      ...
      <form onSubmit={onSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow-lg text-sm">
        ...
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 text-xs">
          ...
          {existingSubImages.length > 0 && (
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                {existingSubImages.map((src, idx) => (
                  <div key={`existing-${idx}`} className="relative group/image">
                    <img src={src} alt={`existing-${idx}`} className="h-24 w-full object-cover rounded-lg" />
                    <button
                      type="button"
                      onClick={(e) => {
                        e.stopPropagation();
                        removeExistingSubImageAt(idx);
                      }}
          ...
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-xl p-4 text-center cursor-pointer bg-white hover:border-[#F272A8]/40 hover:bg-gray-50/50 transition-all duration-200 group"
          >
            {subPreviews.length === 0 && existingSubImages.length === 0 ? (
              <div className="space-y-2">
                <div className="inline-flex p-3 rounded-full bg-[#F272A8]/10">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                  {subPreviews.map((src, idx) => (
                    <div key={`new-${idx}`} className="relative group/image">
              ...
              <div className="space-y-2">
                ...
                <p className="text-gray-500 text-xs">Click to add more images</p>
              </div>
            )}
            <input
              ref={subInputRef}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
          <button
            ...
            className="bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg font-medium hover:bg-gray-300 transition-all duration-200 text-xs"
          >
            Cancel
          </button>
          <button
