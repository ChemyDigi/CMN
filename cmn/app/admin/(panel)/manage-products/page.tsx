...
const DetailRow = ({ label, value }: { label: string; value: any }) => (
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      toolsSnapshot.forEach((docSnap) => {
        allProducts.push({ id: docSnap.id, type: "tool", ...docSnap.data() } as Product);
      });

      const acRefSnapshot = await getDocs(collection(db, "AC-Ref"));
      acRefSnapshot.forEach((docSnap) => {
        allProducts.push({ id: docSnap.id, type: "ac-ref", ...docSnap.data() } as Product);
      });
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 text-black">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 ml-4 text-xs">Manage all tools & AC/Ref products</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow border">
          ...
          <p className="text-2xl font-bold">{products.filter((x) => x.type === "tool").length}</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow border">
          <p className="text-sm text-gray-600">AC & Refrigerator</p>
          <p className="text-2xl font-bold">{products.filter((x) => x.type === "ac-ref").length}</p>
        </div>
      </div>
                  ...
                  <tr key={product.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {product.mainImage ? (
                          <img src={product.mainImage} className="w-12 h-12 object-cover rounded-lg" />
                        ) : (
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">ID: {product.serialId}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">{product.warranty || "Not specified"}</td>
  ...
  <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
    <div className="bg-white w-full max-w-3xl rounded-2xl shadow-xl p-8 max-h-[90vh] overflow-y-auto relative">
      ...
      {/* Title */}
      <h2 className="text-3xl font-bold mb-1">{selectedProduct.productName}</h2>
      <p className="text-gray-500 text-sm mb-6">Product ID: {selectedProduct.serialId}</p>
      ...
      {/* Details Section */}
      <div className="mt-8">
        <h3 className="text-xl font-semibold mb-3 border-b pb-2">Product Details</h3>

        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-3">
            <DetailRow label="Category" value={selectedProduct.category} />
            <DetailRow label="Brand" value={selectedProduct.brand} />
            <DetailRow label="Type" value={getProductTypeLabel(selectedProduct.type)} />
          </div>

          <div className="space-y-3">
            <DetailRow label="Warranty" value={selectedProduct.warranty || "Not specified"} />
            <DetailRow label="Material" value={selectedProduct.material || "Not specified"} />
            <DetailRow
              label="Added On"
              value={
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            {selectedProduct.extraFields.map((field) => (
              <div key={field.id} className="p-4 bg-gray-50 rounded-xl shadow-sm">
                <p className="text-xs font-semibold text-gray-600">{field.name}</p>
